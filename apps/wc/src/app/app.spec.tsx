...
describe('App', () => {
  ...
  it('renders the contributor workflow with the required commitment fields', () => {
    ...
    expect(screen.getByText(/Weekly commitments tied to RCDO outcomes/i)).toBeTruthy();
  ...
  it('creates a fixture-backed commitment when the API is unavailable', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Owner'), { target: { value: 'Avery Chen' } });
    ...
    fireEvent.change(screen.getByLabelText('Planned value'), {
      target: { value: 'Partner digest sent to directors before Friday review' },
    });
    fireEvent.change(screen.getByLabelText('Chess layer'), { target: { value: 'ROOK' } });
    fireEvent.change(screen.getByLabelText('Due date'), { target: { value: '2026-06-06' } });
    fireEvent.click(screen.getByRole('button', { name: /Add commitment/i }));

    await waitFor(() => {
      expect(screen.getAllByText('Publish partner metric digest').length).toBeGreaterThan(0);
    });

    expect(screen.getByText('Partner digest sent to directors before Friday review')).toBeTruthy();
    expect(screen.getAllByText('Avery Chen').length).toBeGreaterThan(0);
    expect(screen.getByText(/alpha workspace while the Cadence API is unavailable/i)).toBeTruthy();
  });

  it('saves reconciliation with carry-forward fallback', async () => {
    renderApp();

    fireEvent.change(screen.getByLabelText('Actual value for Close launch quality gates for onboarding refresh'), {
      target: { value: 'Launch risk remains open for data import validation' },
    });
    fireEvent.click(screen.getByLabelText('Carry forward Close launch quality gates for onboarding refresh'));
    fireEvent.click(screen.getByLabelText('Save reconciliation for Close launch quality gates for onboarding refresh'));

    await waitFor(() => {
      expect(screen.getAllByText('CARRIED FORWARD').length).toBeGreaterThan(0);
    });

    expect(screen.getAllByText(/Launch risk remains open for data import validation/i).length).toBeGreaterThan(0);
  });
  ...
  it('switches to director review and records a manager decision locally', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Review decision for Nikolay Ivanov'), {
    ...
    await waitFor(() => {
      expect(screen.getByText('Escalated: Needs blocker cleared before lock')).toBeTruthy();
    });
  });
});
